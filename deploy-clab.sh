...
run_containerlab_deploy_once() {
  ...
  deploy_pipe="$(mktemp -u "${TMPDIR:-/tmp}/deploy-clab.XXXXXX.pipe")"
  mkfifo "${deploy_pipe}"
  if command -v setsid >/dev/null 2>&1; then
    setsid timeout --foreground "${deploy_timeout_seconds}" \
      containerlab deploy -t "${topology_file}" --reconfigure --timeout "${containerlab_api_timeout}" --max-workers "${deploy_max_workers}" \
      >"${deploy_pipe}" 2>&1 &
  else
  ...
  while true; do
    ...
    if [[ "${line}" =~ (^|[[:space:]])ERRO([[:space:]]|$) ]]; then
      saw_erro=1
      log "Containerlab deploy emitted ERRO; stopping attempt"
      stop_containerlab_deploy "${deploy_pid}"
      break
    fi
  done
...
deploy_lab() {
  ...
  deploy_log="$(mktemp "${TMPDIR:-/tmp}/deploy-clab.XXXXXX.log")"
  for attempt in $(seq 1 "${deploy_attempts}"); do
    if ((attempt > 1)); then
      log "retrying Containerlab deploy after cleanup (attempt ${attempt}/${deploy_attempts})"
    ...
    if grep -q 'failed to Statfs "/proc/0/ns/net"' "${deploy_log}"; then
      log "Containerlab deploy hit netns pid 0 error on attempt ${attempt}/${deploy_attempts}"
      retryable=1
    elif grep -Eq 'failed deploy links.*file exists' "${deploy_log}"; then
      log "Containerlab deploy hit stale link file-exists error on attempt ${attempt}/${deploy_attempts}"
      retryable=1
    elif [[ "${rc}" -eq 124 ]]; then
      log "Containerlab deploy timed out after ${deploy_timeout_seconds}s on attempt ${attempt}/${deploy_attempts}"
      retryable=1
    elif [[ "${rc}" -eq 67 ]]; then
      log "Containerlab deploy idle-timed out after ${deploy_idle_timeout_seconds}s on attempt ${attempt}/${deploy_attempts}"
      retryable=1
    else
      rm -f "${deploy_log}"
      fail "Containerlab deploy failed on attempt ${attempt}/${deploy_attempts}; see log above"
    fi

    if ((retryable != 1 || attempt >= deploy_attempts)); then
      rm -f "${deploy_log}"
      fail "Containerlab deploy did not complete cleanly after ${attempt} attempt(s)"
    fi
  done

  rm -f "${deploy_log}"
  fail "Containerlab deploy did not complete cleanly"
}
