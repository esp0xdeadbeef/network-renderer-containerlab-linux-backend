...
def _controlled_authority_commands(
    ...
    script = [
        ...
        f"ip -6 addr replace {provider_interface6} dev {iface_name} nodad",
        f"ip addr replace {root4}/32 dev {iface_name}",
        f"ip -6 addr replace {root6}/128 dev {iface_name} nodad",
        f"ip addr replace {delegation4}/32 dev {iface_name}",
        f"ip -6 addr replace {delegation6}/128 dev {iface_name} nodad",
        f"ip link set {iface_name} up",
        ...
        *here_document("/run/clabgen-knot.conf", "CLABGEN_KNOT", knot_lines),
        ...
        "rm -f /tmp/clabgen-knot.sock",
        "nohup knotd --config=/run/clabgen-knot.conf --socket=/tmp/clabgen-knot.sock >/tmp/clabgen-knot.log 2>&1 &",
        "knot_pid=$!",
