...
- state=temp-live-verified | target=current CLAB traffic-path p2p realization after manual redeploy | evidence=2026-05-14 running the generated live deploy commands manually inside `s-router-clab-container` (`setup-bridge-links.sh`, `containerlab destroy --topo fabric.no-overlay.clab.yml --cleanup`, `containerlab deploy --topo fabric.no-overlay.clab.yml`) recreated the current rendered fabric and branch samples showed hostile/access/downstream/policy/upstream p2p links with the expected `10.50.0.0/31` lane addresses. | reason=the renderer output can realize the traffic-path p2p contract when redeployed; the remaining durable bug is likely stale deploy cleanup, service sequencing, or verifier strength rather than absent link data.
- state=still-broken | target=deploy verifier false positive | evidence=2026-05-14 `verify-containerlab-deploy.sh` returned success while sampled live routers had only loopback; the same topology became healthy after explicit destroy/deploy. | reason=the generated deploy verifier must detect stale or loopback-only fabric nodes and fail the service/full-loop instead of accepting running containers as enough.
