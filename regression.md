...
- state=solved | target=CLAB linux node restart drops dataplane links | evidence=2026-05-15 Containerlab docs state Linux-kind nodes can lose non-eth0 interfaces after restart; rendered CLAB routers use `network-mode: none`, matching the observed live symptom where fabric routers came back with only `lo`. The renderer now emits `restart-policy: "no"` and a stable `cmd: "/bin/sh -c 'sleep infinity'"` for every generated Linux router node. Focused checks passed: `NETWORK_REPO_DIRECT_TEST_OK=1 bash tests/test-management-eth0-egress-guard.sh`, `NETWORK_REPO_DIRECT_TEST_OK=1 bash tests/test-s-router-clab-overlay-parity.sh`, and `python3 -m py_compile clabgen/s88/site/node_runtime.py`. | reason=Containerlab creates the topology links from explicit CPM output, but Docker auto-restart can discard those non-eth0 links after deploy. The renderer must prevent silent restart/link loss instead of letting the lab treat running loopback-only containers as dataplane proof.
...
- state=solved | `s-router-clab` should be used for live Containerlab validation on this box
...
- state=solved | target=CLAB traffic-path p2p link materialization contract | evidence=2026-05-15 p2p-related failures were: 1. live `clab-fabric-*` routers could be running with only `lo`; 2. rendered commands referenced modeled p2p interfaces such as `eth1`/`eth3` while a restarted Linux-kind container had lost every non-`eth0` veth; 3. the deploy/post-check path accepted container presence before proving p2p links and addresses existed. The renderer-side test contract is now concrete: render a representative CLAB example, assert every generated Linux router has `network-mode: none`, `restart-policy: "no"`, and stable `cmd: "/bin/sh -c 'sleep infinity'"`; assert representative p2p/overlay endpoints are present in `fabric.clab.yml`; assert node exec commands configure the expected p2p interface addresses/routes on those endpoints; and the live lab verifier must fail if any `clab-fabric-*` container has only `lo` or lacks expected p2p addresses after deploy. Focused renderer checks passed: `NETWORK_REPO_DIRECT_TEST_OK=1 bash tests/test-management-eth0-egress-guard.sh` and `NETWORK_REPO_DIRECT_TEST_OK=1 bash tests/test-s-router-clab-overlay-parity.sh`. | reason=the previous vague note did not say whether to test topology generation, Linux restart behavior, or live verifier behavior. The owning renderer fix prevents Linux-kind auto-restart from silently discarding Containerlab-created p2p veths; the live harness must still prove the exact p2p interfaces exist after deploy.
...
- state=solved | A live `s-router-test-three-site` deploy was run through the direct
...
- state=solved | Containerlab created all 28 fabric containers with zero restarts. Data-plane
  links beyond Docker `eth0` were attached, including the overlay links:
  - state=solved | `esp0xdeadbeef-site-a-s-router-core-nebula:eth4` to
    `espbranch-site-b-b-router-core-nebula:eth3`
  - state=solved | `esp0xdeadbeef-site-a-s-router-core-nebula:eth5` to
    `esp0xdeadbeef-site-c-c-router-nebula-core:eth3`
- state=solved | Overlay route probes now resolve through overlay devices instead of Docker
  management:
  - state=solved | branch core to site-A client:
    `10.20.10.1 via 100.96.10.1 dev eth3`
  - state=solved | site-A core to branch client:
    `10.60.10.1 via 100.96.10.2 dev eth4`
  - state=solved | peer overlay host routes:
    `100.96.10.1 dev eth3` and `100.96.10.2 dev eth4`
- state=solved | Live traffic is green for the overlay and DNS service contracts currently
  checked here:
  - state=solved | Site-A client to branch access succeeded
    (`10.20.10.1` to `10.60.10.1`).
  - state=solved | Branch access TCP to site-A DNS connected:
    `10.60.10.1` to `10.20.10.1:53`.
  - state=solved | Branch access UDP DNS to site-A DNS returned a DNS response from
    `10.20.10.1` with `rcode=2` after the modeled public forwarders timed out
    in the CLAB environment.
- state=solved | Containerlab `/etc/hosts` cleanup is a NixOS host realization concern, not a
