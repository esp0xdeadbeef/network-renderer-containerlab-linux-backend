...
assert 'local-zone: "." refuse' in local_config
...
for prefix in local_dns["allowFrom"]:
    ...
    assert f'access-control: "{rendered_prefix}" allow' in local_config
    assert f'access-control: "{rendered_prefix}" refuse_non_local' not in local_config
assert 'access-control: "127.0.0.0/8" allow' in local_config
assert 'access-control: "::1/128" allow' in local_config
...
process_wide_uid_rule = re.compile(
    rf'ip (?:-6 )?rule add uidrange "\$\{{dns_service_uid\}}-\$\{{dns_service_uid\}}" priority {priority} table {table}'
)
assert process_wide_uid_rule.search(core_script) is None
